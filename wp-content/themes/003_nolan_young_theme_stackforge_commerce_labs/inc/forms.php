...
function nolan_young_template_register_form_cpt() {
	register_post_type(
		'nolan_form_submission',
		array(
			'labels'              => array(
				'name'          => __( 'Forms', 'nolan-young-template' ),
				'singular_name' => __( 'Form Submission', 'nolan-young-template' ),
			),
			'public'              => false,
			'show_ui'             => false,
...
function nolan_young_template_submit_form() {
	...
	$post_id = wp_insert_post(
		array(
			'post_type'   => 'nolan_form_submission',
			'post_status' => 'private',
	...
	if ( ! is_wp_error( $post_id ) ) {
		wp_mail( get_option( 'admin_email' ), __( 'New Stackforge Commerce Labs inquiry', 'nolan-young-template' ), "Name: $name\nEmail: $email\nPhone: $phone\nService: $service\n\n$message" );
	}
...
function nolan_young_template_forms_menu() {
	add_menu_page( __( 'Forms', 'nolan-young-template' ), __( 'Forms', 'nolan-young-template' ), 'manage_options', 'stackforge-forms', 'nolan_young_template_forms_page', 'dashicons-feedback', 58 );
}
...
function nolan_young_template_forms_page() {
	...
	$submissions = get_posts( array( 'post_type' => 'nolan_form_submission', 'post_status' => 'private', 'numberposts' => 50, 'orderby' => 'date', 'order' => 'DESC' ) );
	echo '<div class="wrap"><h1>' . esc_html__( 'Forms', 'nolan-young-template' ) . '</h1><p><a class="button button-primary" href="' . esc_url( wp_nonce_url( admin_url( 'admin.php?page=stackforge-forms&export=1' ), 'stackforge_forms_export' ) ) . '">' . esc_html__( 'Export CSV', 'nolan-young-template' ) . '</a></p><table class="widefat striped"><thead><tr><th>' . esc_html__( 'Date', 'nolan-young-template' ) . '</th><th>' . esc_html__( 'Type', 'nolan-young-template' ) . '</th><th>' . esc_html__( 'Name', 'nolan-young-template' ) . '</th><th>' . esc_html__( 'Email', 'nolan-young-template' ) . '</th><th>' . esc_html__( 'Message', 'nolan-young-template' ) . '</th></tr></thead><tbody>';
...
function nolan_young_template_export_forms() {
	...
	header( 'Content-Disposition: attachment; filename=stackforge-form-submissions.csv' );
	$output = fopen( 'php://output', 'w' );
	fputcsv( $output, array( 'Date', 'Type', 'Name', 'Email', 'Phone', 'Service', 'Message' ) );
	foreach ( get_posts( array( 'post_type' => 'nolan_form_submission', 'post_status' => 'private', 'numberposts' => -1 ) ) as $submission ) {
