...
document.querySelectorAll('[data-service-tab]').forEach((button) => {
  button.addEventListener('click', () => {
    const key = button.dataset.serviceTab;
    const wrap = button.closest('.service-selector');
    if (!wrap) return;
    wrap.querySelectorAll('[data-service-tab]').forEach((item) => {
      const active = item === button;
      item.classList.toggle('is-active', active);
      item.setAttribute('aria-pressed', String(active));
    });
    wrap.querySelectorAll('[data-service-panel]').forEach((panel) => {
...
document.querySelectorAll('[data-season-tab]').forEach((button) => {
  button.addEventListener('click', () => {
    const key = button.dataset.seasonTab;
    const wrap = button.closest('[data-season-tabs]');
    if (!wrap) return;
    wrap.querySelectorAll('[data-season-tab]').forEach((item) => {
      const active = item === button;
      item.classList.toggle('is-active', active);
      item.setAttribute('aria-pressed', String(active));
    });
    wrap.querySelectorAll('[data-season-panel]').forEach((panel) => {
      const active = panel.dataset.seasonPanel === key;
      panel.hidden = !active;
      panel.classList.toggle('is-active', active);
    });
  });
...
document.querySelectorAll('[data-estimate-checklist] input[type="checkbox"]').forEach((checkbox) => {
