...
package_theme() {
  bash "$root_dir/scripts/packaging/package-generated-wordpress-theme-zip.sh" "$slug"
}

validate_theme() {
  THEME_FACTORY_SKIP_PROMPT_LIFECYCLE=1 bash "$root_dir/scripts/validation/validate-generated-theme-all.sh" "$slug" 2>&1 | tee "$run_dir/validation-output.txt"
}

run_ollama_stage() {
  local stage="$1"
  local prompt_path="$2"
  OLLAMA_MODEL="$ollama_model" bash "$root_dir/scripts/ollama/run-ollama-generation-stage.sh" "$stage" "$slug" "$prompt_path" "$root_dir"
}

run_codex_final_pass() {
  local prompt_path="$1"
  CODEX_COMMAND="$codex_command" bash "$root_dir/scripts/codex/run-codex-automation-pass.sh" "$slug" "$prompt_path"
}
...
bash "$root_dir/scripts/validation/validate-prompt-lifecycle-archives.sh" "$slug" 2>&1 | tee -a "$run_dir/validation-output.txt"
