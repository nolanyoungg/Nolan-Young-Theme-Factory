...
function nolan_young_template_register_form_cpt() {
	register_post_type(
		'atlasframe_form',
		array(
			'labels'              => array( 'name' => __( 'Forms', '007-nolan-young-theme-atlasframe-digital' ), 'singular_name' => __( 'Form Submission', '007-nolan-young-theme-atlasframe-digital' ) ),
			'public'              => false,
			'show_ui'             => false,
...
function nolan_young_template_submit_form() {
	if ( ! isset( $_POST['nolan_young_template_form_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nolan_young_template_form_nonce'] ) ), 'nolan_young_template_form' ) ) {
		wp_die( esc_html__( 'The form security check failed.', '007-nolan-young-theme-atlasframe-digital' ), 403 );
	}
	...
	if ( ! is_wp_error( $post_id ) ) {
		wp_mail( get_option( 'admin_email' ), __( 'New Atlasframe Digital inquiry', '007-nolan-young-theme-atlasframe-digital' ), "Name: $name\nEmail: $email\nPhone: $phone\nService: $service\n\n$message" );
	}
...
function nolan_young_template_forms_menu() {
	add_menu_page( __( 'Forms', '007-nolan-young-theme-atlasframe-digital' ), __( 'Forms', '007-nolan-young-theme-atlasframe-digital' ), 'manage_options', 'atlasframe-forms', 'nolan_young_template_forms_page', 'dashicons-feedback', 58 );
}
add_action( 'admin_menu', 'nolan_young_template_forms_menu' );

function nolan_young_template_forms_page() {
	if ( ! current_user_can( 'manage_options' ) ) {
		wp_die( esc_html__( 'You do not have permission to view submissions.', '007-nolan-young-theme-atlasframe-digital' ) );
	}
	if ( isset( $_GET['export'] ) && check_admin_referer( 'atlasframe_forms_export' ) ) {
		nolan_young_template_export_forms();
	}
	$submissions = get_posts( array( 'post_type' => 'atlasframe_form', 'post_status' => 'private', 'numberposts' => 50, 'orderby' => 'date', 'order' => 'DESC' ) );
	echo '<div class="wrap"><h1>' . esc_html__( 'Forms', '007-nolan-young-theme-atlasframe-digital' ) . '</h1>';
	echo '<p><a class="button button-primary" href="' . esc_url( wp_nonce_url( admin_url( 'admin.php?page=atlasframe-forms&export=1' ), 'atlasframe_forms_export' ) ) . '">' . esc_html__( 'Export CSV', '007-nolan-young-theme-atlasframe-digital' ) . '</a></p>';
	echo '<table class="widefat striped"><thead><tr><th>' . esc_html__( 'Date', '007-nolan-young-theme-atlasframe-digital' ) . '</th><th>' . esc_html__( 'Type', '007-nolan-young-theme-atlasframe-digital' ) . '</th><th>' . esc_html__( 'Name', '007-nolan-young-theme-atlasframe-digital' ) . '</th><th>' . esc_html__( 'Email', '007-nolan-young-theme-atlasframe-digital' ) . '</th><th>' . esc_html__( 'Message', '007-nolan-young-theme-atlasframe-digital' ) . '</th></tr></thead><tbody>';
...
function nolan_young_template_export_forms() {
	...
	header( 'Content-Disposition: attachment; filename=atlasframe-form-submissions.csv' );
	$output = fopen( 'php://output', 'w' );
	fputcsv( $output, array( 'Date', 'Type', 'Name', 'Email', 'Phone', 'Service', 'Message' ) );
	foreach ( get_posts( array( 'post_type' => 'atlasframe_form', 'post_status' => 'private', 'numberposts' => -1 ) ) as $submission ) {
