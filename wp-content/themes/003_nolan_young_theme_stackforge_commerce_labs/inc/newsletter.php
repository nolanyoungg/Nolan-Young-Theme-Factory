...
function nolan_young_template_register_newsletter_cpt() {
	register_post_type(
		...
		array(
			'labels'              => array(
				'name'          => __( 'Newsletter', 'nolan-young-template' ),
				'singular_name' => __( 'Subscriber', 'nolan-young-template' ),
			),
			'public'              => false,
			'show_ui'             => false,
...
function nolan_young_template_newsletter_signup() {
	...
	$existing = get_posts( array( 'post_type' => 'nolan_subscriber', 'post_status' => 'private', 'numberposts' => 1, 'meta_key' => '_email', 'meta_value' => $email ) );
	if ( $existing ) {
		update_post_meta( $existing[0]->ID, '_status', 'Active' );
		update_post_meta( $existing[0]->ID, '_unsubscribed_at', '' );
		update_post_meta( $existing[0]->ID, '_first_name', $name );
	} else {
		wp_insert_post( array( 'post_type' => 'nolan_subscriber', 'post_status' => 'private', 'post_title' => $email, 'meta_input' => array( '_email' => $email, '_first_name' => $name, '_status' => 'Active', '_token' => wp_generate_password( 32, false, false ), '_unsubscribed_at' => '' ) ) );
	}
...
function nolan_young_template_newsletter_menu() {
	add_menu_page( __( 'Newsletter', 'nolan-young-template' ), __( 'Newsletter', 'nolan-young-template' ), 'manage_options', 'stackforge-newsletter', 'nolan_young_template_newsletter_page', 'dashicons-email-alt2', 59 );
}
...
function nolan_young_template_newsletter_page() {
	...
	$subscribers = get_posts( array( 'post_type' => 'nolan_subscriber', 'post_status' => 'private', 'numberposts' => 100 ) );
	echo '<div class="wrap"><h1>' . esc_html__( 'Newsletter', 'nolan-young-template' ) . '</h1><table class="widefat striped"><thead><tr><th>Email</th><th>Name</th><th>Status</th><th>Signup date</th><th>Unsubscribed</th></tr></thead><tbody>';
