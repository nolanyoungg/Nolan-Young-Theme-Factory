...
function finalizeTheme(themeSlug, templateName, reportDir, state, validationFinalPath) {
  ...
  const preview = run('node', ['scripts/generate-static-preview.js', themeSlug]);
  ...
  const gallery = run('node', ['scripts/rebuild-preview-gallery.js']);
...
if (mode === 'codex-only') {
  ...
  run('node', ['scripts/create-codex-theme-brief.js', mode, themeSlug, templateName, promptFile, path.join(reportDir, 'generation-brief.path.txt'), path.join(reportDir, 'template.manifest.json'), validationBeforePath, codexModel, codexReasoning, codexBrief]);
}
