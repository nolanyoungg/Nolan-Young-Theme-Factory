...
Create and preserve this exact required baseline structure:
...
Every listed file and directory is required.

Do not rename, omit, or move any required path.

Additional files may be added only when they are genuinely necessary. Additional files must not replace, duplicate, or conflict with the required structure.

The homepage must be implemented in the root `front-page.php` file. Do not create a homepage page template under `page-templates/`.

### Required File Responsibilities

`functions.php` must remain a small bootstrap file that loads the modular files from `inc/`.

`inc/setup.php` must register theme supports, navigation menus, image sizes, editor support, and other theme initialization.

`inc/enqueue.php` must enqueue `assets/css/bundle.css` and `assets/js/bundle.js`.

`inc/template-tags.php` must contain reusable presentation helpers intended for templates.

`inc/helpers.php` must contain general-purpose internal helper functions.

`inc/custom-post-types.php` must contain custom post type and taxonomy registration when required by the theme.

`inc/customizer.php` must contain WordPress Customizer settings and controls.

`inc/forms.php` must contain form registration, validation, storage, notifications, exports, and administration logic.

`inc/newsletter.php` must contain newsletter signup, subscriber status, unsubscribe, export, and administration logic.

`inc/policy-routing.php` must contain policy-template routing and related policy-page behavior.

`front-page.php` must render the complete homepage.

`403.php` must provide an appropriate forbidden-access template that does not expose sensitive information.

`blocks/README.md` must explain the block directory and document whether custom blocks are currently implemented.

`assets/icons/README.md` must document the local icon rules and icon usage.

`docs/getting-started.md` must explain installation, activation, build commands, required pages, and initial setup.

`docs/customization.md` must explain supported customization points without encouraging direct edits to compiled files.

`accessibility/README.md` must document keyboard behavior, focus management, semantic markup, contrast, motion, accordions, navigation panels, forms, and testing expectations.
...
|---|---|
| Required structure | Every required file and directory from the supplied theme structure exists at the exact path |
| No conflicting structure | No required path was renamed, moved, replaced, or duplicated under an alternate name |
