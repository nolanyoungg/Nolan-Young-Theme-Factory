...
[ -n "$theme_slug" ] && [ -n "$prompt_file" ] || fail "Usage: bash scripts/modes/ollama-only/run-ollama-quality-repair-pass.sh <theme-slug> <prompt-file> [model]"
...
EOF
  ...
  if ! node scripts/ai-output/apply-theme-file-blocks.js "$raw_output" "wp-content/themes/$theme_slug"; then
