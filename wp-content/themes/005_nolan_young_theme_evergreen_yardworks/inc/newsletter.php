<?php
/**
 * Seasonal reminder signup management.
 *
...
function nolan_young_template_register_newsletter_cpt() {
	register_post_type(
		'yardworks_subscriber',
		array(
			'labels'              => array( 'name' => __( 'Seasonal Reminders', '005-nolan-young-theme-evergreen-yardworks' ), 'singular_name' => __( 'Subscriber', '005-nolan-young-theme-evergreen-yardworks' ) ),
			'public'              => false,
			'show_ui'             => false,
...
function nolan_young_template_newsletter_signup() {
	if ( ! isset( $_POST['nolan_young_template_newsletter_nonce'] ) || ! wp_verify_nonce( sanitize_text_field( wp_unslash( $_POST['nolan_young_template_newsletter_nonce'] ) ), 'nolan_young_template_newsletter' ) ) {
		wp_die( esc_html__( 'Seasonal reminder security check failed.', '005-nolan-young-theme-evergreen-yardworks' ), 403 );
	}
	...
	$existing = get_posts( array( 'post_type' => 'yardworks_subscriber', 'post_status' => 'private', 'numberposts' => 1, 'meta_key' => '_email', 'meta_value' => $email ) );
	...
	if ( $existing ) {
		...
		wp_insert_post( array( 'post_type' => 'yardworks_subscriber', 'post_status' => 'private', 'post_title' => $email, 'meta_input' => array( '_email' => $email, '_first_name' => $name, '_status' => 'Active', '_token' => $unsubscribe_hash ) ) );
	}
...
function nolan_young_template_newsletter_menu() {
	add_menu_page( __( 'Seasonal Reminders', '005-nolan-young-theme-evergreen-yardworks' ), __( 'Seasonal Reminders', '005-nolan-young-theme-evergreen-yardworks' ), 'manage_options', 'evergreen-yardworks-reminders', 'nolan_young_template_newsletter_page', 'dashicons-email-alt2', 59 );
}
...
function nolan_young_template_newsletter_page() {
	...
	if ( isset( $_GET['export'] ) && check_admin_referer( 'yardworks_reminders_export' ) ) {
		nolan_young_template_export_newsletter();
	}
	$subscribers = get_posts( array( 'post_type' => 'yardworks_subscriber', 'post_status' => 'private', 'numberposts' => 100 ) );
	echo '<div class="wrap"><h1>' . esc_html__( 'Evergreen Yardworks Seasonal Reminders', '005-nolan-young-theme-evergreen-yardworks' ) . '</h1><p><a class="button button-primary" href="' . esc_url( wp_nonce_url( admin_url( 'admin.php?page=evergreen-yardworks-reminders&export=1' ), 'yardworks_reminders_export' ) ) . '">' . esc_html__( 'Export CSV', '005-nolan-young-theme-evergreen-yardworks' ) . '</a></p><table class="widefat striped"><thead><tr><th>Email</th><th>Name</th><th>Status</th><th>Signup date</th><th>Unsubscribed</th></tr></thead><tbody>';
...
function nolan_young_template_export_newsletter() {
	...
	header( 'Content-Disposition: attachment; filename=evergreen-yardworks-seasonal-reminders.csv' );
	$output = fopen( 'php://output', 'w' );
	fputcsv( $output, array( 'Email', 'First name', 'Status', 'Signup date', 'Unsubscribed at' ) );
	foreach ( get_posts( array( 'post_type' => 'yardworks_subscriber', 'post_status' => 'private', 'numberposts' => -1 ) ) as $subscriber ) {
